...
## test to export network and then read it to save for later and reuse or display for analysis
# testos <- as.matrix(network)
# write.csv(testos, file = "random network")
# import <- read.csv("random network")
# import <- subset(import, select = "V1")
# intermediary <- as.matrix(import)
# back <- array(data = intermediary, dim = dimensions)
# all.equal(network, back)
...
for (i in seq(20)) {
  ...
  ## calculate p(Y = y) which is the denominator / normalizer
  for (n in seq(1000)) { sampledValues[n,5] <- sampledValues[n,5] / Y1 }
...
write(errors_ess, "random network ess errors")
write(samples, "random network ess samples")
write(ess_exploration_factor, "random network ess exploration factor")
...
write(errors_acceptance, "random network acceptance errors")
write(samples, "random network acceptance samples")
write(ar_exploration_factor, "random network acceptance exploration factor")
