...
# each row has to be a distribution
network <- bayesianNetwork(dimensions)
network <- array(data = rep(c(0), 10000), dim = dimensions)
network[,1,,] <- 0.49996
network[,2,,] <- 0.00001
network[,3,,] <- 0.00001
network[,4,,] <- 0.00001
...
write(errors_ess, "valley ess errors")
write.csv2(samples, "valley network ess samples")
write(ess_exploration_factor, "valley ess exploration factor")
...
write(errors_acceptance, "valley acceptance errors j6")
write(ar_exploration_facto, "valley acceptance rate exploration factor")
write.csv2(samples, "valley network acceptance samples")
